...
- macOS：下载 `Hider-*-mac-arm64.dmg` 后拖入应用程序目录。
- Windows：下载 `Hider-*-win-x64-setup.exe` 后按安装向导完成安装。
...
| 搜索模式 | 输入关键词后返回相关真实链接，打开结果后可继续阅读模式或提取正文。 |
...
搜索模式会在本机请求搜索引擎结果页并整理真实链接，不经过 Hider 官方服务器。只有在你打开具体结果时，Hider 才会访问对应网站。网页内容提取仅用于当前阅读界面展示。

## 许可证

本项目代码基于 GNU Affero General Public License v3.0 only（AGPL-3.0-only）发布，详见 [LICENSE](LICENSE)。

Hider 名称、图标和品牌资产不随代码授权自动授予商标或品牌使用权。未经许可，不得使用 Hider 名称、图标或相似标识发布可能让用户误认为是官方版本的构建。
